...
def add_game_to_sunshine_api(
    ...
    """Add or update a game in the active server configuration using the API.

    When index is None, the game is created as a new entry (index=-1).
    When index is provided, the existing entry at that position is updated.
    """
    payload = {
        "name": game_name,
        "output": "",
    ...
    action = "Updated" if index is not None else "Added"
    _, error = sunshine_api_request("POST", "/api/apps", json=payload)
    if error:
        print(f"Error {action.lower()} {game_name} to {get_server_display_name()} via API: {error}")
    else:
        print(f"{action} {game_name} to {get_server_display_name()}.")
...
def add_game_to_sunshine(
    ...
    """Add or update a game in the Sunshine configuration.

    When index is None, the game is created as a new entry.
    When index is provided, the existing entry at that position is updated.
    """
    ...
    # Detect Steam games (before flatpak wrapping)
    is_steam = runner == "Steam" or ("steam://run/" in cmd)
    ...
    # For Steam games, use start-steam-game.sh via detached instead of raw cmd
    if is_steam:
        cmd = ""
        detached = [str(SWAY_SUNSHINE_DIR / "start-steam-game.sh") + " " + str(game_id)]
    # For Lutris games, use start-lutris-game.sh via detached instead of raw cmd
    elif is_lutris:
        cmd = ""
    ...
    if enable_display:
        prep_cmd = [
            {
                "do": str(SWAY_SUNSHINE_DIR / "restore-default-sink.sh"),
                "undo": "",
            },
            {
                "do": str(SWAY_SUNSHINE_DIR / "set-resolution.sh"),
                "undo": "",
            },
        ]
        # Set the correct undo for the resolution entry based on game type
        if is_steam:
    ...
    add_game_to_sunshine_api(game_name, cmd, image_path, prep_cmd=prep_cmd, detached=detached, index=index)
